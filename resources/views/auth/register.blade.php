...
<main class="flex min-h-screen items-center justify-center bg-paper px-4 pb-12 pt-28 sm:pt-32">
  <div class="grid w-full max-w-4xl overflow-hidden rounded-2xl border border-line bg-surface shadow-rf md:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
    ...
    {{-- ANDEN: la misma red de teleferico que el login, ahora abordas por primera vez --}}
    <aside class="relative flex flex-col gap-8 bg-ink px-7 py-9 text-white sm:px-9" aria-hidden="true">
      ...
      <ol class="rf-linea relative m-0 flex flex-1 list-none flex-col justify-center gap-7 p-0 pl-7">
        ...
        <li class="rf-estacion rf-estacion--activa" style="--i:3">
          ...
          <span class="rf-sub">Empieza aqui</span>
        </li>
      </ol>
    ...
    {{-- DESTINO: el formulario de registro --}}
    <section class="flex flex-col justify-center gap-6 px-7 py-9 sm:px-10">
      <header class="grid gap-1.5">
        <p class="m-0 text-xs font-bold uppercase tracking-[0.2em] text-magenta">Nueva cuenta</p>
        <h1 class="m-0 text-3xl font-extrabold leading-tight text-ink sm:text-4xl">Sube a la red</h1>
        <p class="m-0 text-muted">Crea tu cuenta para opinar y participar en Ruta Facil.</p>
      </header>

      @if ($errors->any())
        <div role="alert" class="grid gap-1 rounded-lg border border-red/30 bg-red/10 px-4 py-3 text-sm font-semibold text-red">
      ...
      <form method="POST" action="{{ route('register') }}" class="grid gap-4">
        ...
        <div class="grid gap-1.5">
          <label for="name" class="text-xs font-bold uppercase tracking-[0.14em] text-ink">Nombre completo</label>
          <input id="name" type="text" name="name" value="{{ old('name') }}" required autocomplete="name"
                 autofocus placeholder="Tu nombre y apellido"
        ...
        <div class="grid gap-1.5">
          <label for="password" class="flex items-baseline gap-2 text-xs font-bold uppercase tracking-[0.14em] text-ink">
            Contrasena
            <span class="font-semibold normal-case tracking-normal text-muted">minimo 8 caracteres</span>
          </label>
        ...
        <div class="grid gap-1.5">
          <label for="password_confirmation" class="text-xs font-bold uppercase tracking-[0.14em] text-ink">Confirmar contrasena</label>
      ...
      <p class="m-0 text-center text-sm text-muted">
        Ya tienes cuenta?
        <a href="{{ route('login') }}" class="font-extrabold text-green underline-offset-2 hover:underline">Inicia sesion</a>
      </p>
    </section>
  </div>
</main>
